...
public class Damped1D extends JComponent implements Runnable{
	...
	static double x = 0, v = 0, ag =50, a, t = 0.05, m = 10, k = 2, time = 0, st, mv = Integer.MIN_VALUE, tp, mx = Integer.MIN_VALUE, radius = 25, ground = 800, offset=800, damping =0.5;
	public static void main(String args[]) {
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setBounds(0,0,width+6+6+6,height+29+29+3);
		frame.getContentPane().add(new Damped1D());
	...
	public void paint(Graphics g)
	{
    	...
    	a = ag - k * (x-(hinge+nl))/m - damping*v/m;
    	v = v + a*t;
    	vv.add(v);
    	mv = Math.max(v, mv);
    	x = x + v*t;
